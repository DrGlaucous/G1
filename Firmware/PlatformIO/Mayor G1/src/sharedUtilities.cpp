...
void SelectFireTimeout(void)
{
  ...
  if (proceed == true)//if the pusher has started (proceed == true when flywheels have reached speed)
  {
    ...
    MillisecondTicks % 10 == 0 &&//on an 10 millisecond interval
    MillisecondTicks != LastMicrosecondTicks//only do it one time per interval
    )
    {
      timer += 1;
    }
    if (timer >= ((pow(delayCurve ,6 ) / ArdAnalogRead(PUSH_MODE_PIN)) / 10 ) )//a function to roughly map timeout to the expected time to complete a push cycle
    {
      clicked = false;
      timer = 0;
      noTone(STEP_PIN);
...
//handle the flywheels (requires a 10 millisecond interval)
void HandleESC(void)
{


    if(MillisecondTicks % 10 == 0 &&//on an 10 millisecond interval
    MillisecondTicks != LastMicrosecondTicks//only do it one time per interval
    )
    {
